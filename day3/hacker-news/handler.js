...
exports.doSubmit = function (req, res) {
  // 对于表单 post 请求体来说，请求体中的数据可能很大
  // 所以表单 post 提交会把要提交的数据进行分块传输

  // 在服务器端，也需要一块一块的接收表单 post 请求体数据
  // 通过监听 req 请求对象的 data 事件和 end 事件就可以接收了
  var count = 0
  var data = ''
  // 二进制数据有一个 length 属性，length 属性获取到的就是二进制数据的字节
  req.on('data', function (chunk) {
    data += chunk
  })

  req.on('end', function () {
    // 通过使用内置模块 querystring 的 parse 将一个查询字符串转为一个对象
...
// 1. 读取模板字符串 文件名不一样
// 2. 产生要注入的模板数据 数据也不一样
// 3. 使用模板引擎将解析替换过后的字符串发送给客户端 发送的数据也不一样
